# emprestimos.py — ~200 linhas, tudo junto
...
class Sistema:
    ...
    def devolver(self, emprestimo_id):
        ...
        # if/elif por tipo — violação de OCP, duplicado abaixo
        multa = 0
        if atraso > 0:
        ...
        # notificação de novo misturada aqui
    ...
    def listar_atrasados(self):
        ...
        for e in emprestimos_registrados:
            if not e["devolvido"] and e["data_devolucao"] < hoje:
                ...
                # notificação pela terceira vez
                print(f"[EMAIL] {e['usuario_email']} — você está em atraso!")

# menu misturado no mesmo arquivo
def main():
    s = Sistema()
    while True:
        ...
        op = input("Opção: ")
        if op == "1":
            s.registrar(int(input("ID equipamento: ")), input("Nome: "),
                        input("Email: "), int(input("Dias: ")))
        elif op == "2":
